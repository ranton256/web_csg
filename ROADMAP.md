...
| | Item | From |
|---|---|---|
| ☑ | ~~Deploy the corrected `.vercelignore` to production: until `npx vercel deploy --prod` runs, <https://csg.ranton.org> serves the earlier upload, which still publishes `playwright.config.js`~~ Deployed by the owner, 2026-09-15: `playwright.config.js` and `test/` now return 404 and the app returns 200, so the live published set matches CONSTRAINTS §1 | `deploy-csg-subdomain` Critic rounds 2 (N5) and 3 (F2) |
...
| ☑ | ~~Test D28 across a reload: open a file with `\r\n` line breaks, reload, then choose an example, and expect no prompt. A mutant that saves the raw text as the stored baseline survives today's tests~~ Delivered on branch `d28-reload-test`: `e2e/persistence.spec.js` "a file with \r\n line breaks, reloaded, still replaces without a prompt". It passes on all three engines, and fails on all three against that mutant | `m6-persistence-and-examples` Critic round 2 (N1, low) |
