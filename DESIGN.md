...
Scenario: Keyboard users are not trapped
  ...
  When Esc is pressed and then Tab, or Shift+Tab
  Then focus moves to the next, or previous, element on the page, and the text is unchanged
...
| ~~D17~~ | **Resolved 2026-09-11 (owner, in the D17 interview before M6):** identifiers stay ASCII letters, digits, and `_`, and whitespace stays the ASCII space, tab, and line-break characters, so `é` and a non-breaking space remain errors. One U+FEFF at the very start of the source is skipped and takes no column; anywhere else it is an error. Comments may contain any character. An unexpected character's diagnostic names it: `'$'` for visible ASCII, `'é' (U+00E9)` for visible non-ASCII, and `U+XXXX` for an invisible character (categories Zs, Zl, Zp, Cc, Cf), plus a name for 14 common ones, for example `U+00A0 (no-break space)` and `U+FEFF (byte-order mark)`. Recorded in the `modeling-language` spec and §8 Modeling language by M6. | — | — || ~~D29~~ | **Resolved 2026-09-15 (owner):** the app is published as a static site on Vercel at <https://csg.ranton.org>, so it can be opened from a link without installing anything. Deployment is manual (`npx vercel deploy --prod`), not driven by a Git integration, because the app changes rarely and every deploy should follow a green gate. Hosting is a delivery detail: it adds no build step, no server code, and no dependency for anything under `src/`. Recorded in CONSTRAINTS §1 and §4. | — | — |
