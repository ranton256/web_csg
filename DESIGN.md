...
| Supported scene scale | Dimensions, coordinates, and derived feature sizes (wall thicknesses, gaps, overlaps, distances from the camera to visible surfaces) with magnitude in `[1e-3, 2e7]` world units | **Final** (M4). This is the range the scaled-render check exercises, from the 0.001 overhang at ×1e-3 to the 1.6e7 camera coordinate at ×1e5 (D7). Because `ε` is absolute, scaling leaves an image unchanged only while every such size stays in the range. Outside it, rendering is best effort (D16) |
...
- After every Boolean operation: intervals of length ≤ `ε` are dropped
  ...
  cutter overlaps or meets the base, while a cutter that stops short of the
  base, even by a gap ≤ `ε`, leaves the base boundary (D21).
...
| ~~D21~~ | **Resolved 2026-09-10 (owner, after M4 Critic round 1):** the difference boundary rule applies where a cutter overlaps or meets the base. A cutter that stops short of the base, even by a gap ≤ `ε`, does not touch it, so the base boundary stays. Recorded in §8 Ray–solid intervals and the `ray-intervals` spec. | — | — |
