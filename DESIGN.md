...
- After every Boolean operation: intervals of length ≤ `ε` are dropped
  ...
  a side line has real length and is a hit, per D20); in a union, intervals whose gap
  is ≤ `ε` are merged (flush faces leave no seam); in a difference, a cutter
  boundary within `ε` of a base boundary removes that base boundary.
