...
| ~~D29~~ | **Resolved 2026-09-15 (owner):** the app is published as a static site on Vercel at <https://csg.ranton.org>, so a reader can open it from a link without installing anything, and deployment is manual (`npx vercel deploy --prod`) rather than driven by a Git integration. **The writer's rules, following from those decisions:** every deploy follows a green gate (CONSTRAINTS §4), and hosting stays a delivery detail — it adds no build step, no server code, and nothing under `src/` may depend on it. Recorded in CONSTRAINTS §1 and §4. | — | — |
