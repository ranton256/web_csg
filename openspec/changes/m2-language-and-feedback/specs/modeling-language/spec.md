...
**Reason**: M2 completes the grammar; the M1 subset no longer describes the language.
**Migration**: See the new "Statements" requirement, which covers every statement form.

### Requirement: Expressions of the M1 subset
**Reason**: Binary arithmetic and parentheses are now supported, so the M1 rule that rejected them no longer holds.
**Migration**: See the new "Expressions" requirement (operators, precedence, operand kinds, division by zero).
