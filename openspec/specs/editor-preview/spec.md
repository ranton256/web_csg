...
The in-browser page: a source editor with a line-number gutter and clickable
diagnostics, beside a preview panel that fills the rest of the window, with a
draggable divider between them. Rebuild timing and the stale state are
specified in `live-rebuild` and `stale-preview`.
