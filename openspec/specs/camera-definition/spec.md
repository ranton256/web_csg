# camera-definition Specification
...
range check.
- **Distance overflow:** when the distance from `position` to `lookAt` is too
  large to compute as a finite number, the evaluator SHALL report a
  diagnostic at `lookAt` saying that position and lookAt are too far apart.
  It SHALL NOT report up as zero or parallel because of that overflow.
- **Up overflow:** when the length of `up` is too large to compute as a
  finite number, the evaluator SHALL report a diagnostic at `up` saying up
  is too large. It SHALL NOT report up as parallel.
- **Up underflow:** when `up` has a nonzero component but its length
  underflows to 0, the evaluator SHALL report a diagnostic at `up` saying up
  is too small. It SHALL NOT report up as zero.
...
#### Scenario: An up vector too small is reported clearly
- **WHEN** the camera has `position: [0, -100, 0]`, `lookAt: [0, 0, 0]`, and `up: [0, t, t]`, where `t` is `0.` followed by 199 zeros and a `1`
- **THEN** the only camera diagnostic is at `up` and says up is too small
